...
public class CheckPlayerGUI {
	...
	public static boolean CheckName(String PlayerCurrentName) {
		...
		for(String path : Wardrobe.Page_1.getConfig().getConfigurationSection("").getKeys(false)) {
			if (Wardrobe.Page_1.getConfig().getString(path + ".name").contains(PlayerCurrentName)) {
				Path = path;
				return true;
			}
		}
